...
import { ArrowRight, BarChart3, CheckCircle2, Clock3, Copy, LockKeyhole, Play, RefreshCcw, Sparkles, Target, Trophy } from 'lucide-react';
...
export default function BenchmarkExperience({
  ...
  useEffect(() => {
    ...
    const userReport = readSavedBenchmarkReport(user?.id);
    const resolvedReport = userReport ?? anonymousReport;
    if (resolvedReport) {
      setSavedReport(resolvedReport);
    }
    if (user?.id && anonymousReport && !userReport) {
    ...
    <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-[0_20px_60px_rgba(15,23,42,0.08)] sm:p-8">
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-3">
        <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4">
          <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-slate-500"><Clock3 className="h-4 w-4 text-sky-600" />10-minute window</div>
          <div className="mt-2 text-sm leading-6 text-slate-600">Fast enough for onboarding, real enough to be useful.</div>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4">
          <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-slate-500"><BarChart3 className="h-4 w-4 text-emerald-600" />Skill report</div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4">
          <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-slate-500"><Target className="h-4 w-4 text-violet-600" />Outcome-first</div>
                ...
                return (
                  <div key={title as string} className="rounded-2xl border border-slate-200 bg-slate-50 px-5 py-4">
                    <div className="flex items-start gap-4">
                      <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-white shadow-sm"><Icon className="h-5 w-5 text-slate-900" /></div>
                      <div>
          ...
          {view === 'assessment' && activeQuestion ? (
            <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-[0_20px_60px_rgba(15,23,42,0.08)] sm:p-8">
              <div className="flex flex-col gap-4 border-b border-slate-200 pb-6 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="inline-flex items-center gap-2 rounded-full bg-slate-950 px-4 py-2 text-sm font-medium text-white"><Clock3 className="h-4 w-4" />{formatDuration(secondsLeft)} left</div>
              </div>
                    ...
                    return (
                      <button
                        ...
                        className={`rounded-2xl border px-5 py-4 text-left transition ${
                          isSelected ? 'border-slate-950 bg-slate-950 text-white shadow-lg shadow-slate-900/10' : 'border-slate-200 bg-white hover:border-slate-300 hover:bg-slate-50'
                        }`}
                      >
              ...
              <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <button type="button" onClick={resetBenchmark} className="inline-flex items-center justify-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-medium text-slate-700 transition hover:bg-slate-50"><RefreshCcw className="h-4 w-4" />Restart</button>
                  ...
                  {!user ? (
                    <div className="rounded-[1.75rem] border border-violet-200 bg-violet-50/80 p-5">
                      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                          <div className="flex items-center gap-2 text-sm font-semibold uppercase tracking-[0.18em] text-violet-700"><LockKeyhole className="h-4 w-4" />Unlock full roadmap</div>
                    ...
                    <div className="rounded-[1.75rem] border border-emerald-200 bg-emerald-50/70 p-5">
                      <div className="flex items-center gap-2 text-sm font-semibold uppercase tracking-[0.18em] text-emerald-700"><CheckCircle2 className="h-4 w-4" />Saved to your workspace</div>
                      <p className="mt-2 text-sm leading-6 text-emerald-900">Your report is saved locally for this account so you can revisit it while working through practice and duels.</p>
                    </div>
                  )}
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <button type="button" onClick={resetBenchmark} className="inline-flex items-center justify-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-medium text-slate-700 transition hover:bg-slate-50"><RefreshCcw className="h-4 w-4" />Retake benchmark</button>
                    <Link to="/report-sample" className="inline-flex items-center justify-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-medium text-slate-700 transition hover:bg-slate-50"><span>View sample report</span><ArrowRight className="h-4 w-4" /></Link>
                  </div>
                </div>
              }
            />
          ...
          {view === 'setup' ? (
            <div className="rounded-[2rem] border border-dashed border-slate-300 bg-white/80 p-6 sm:p-8">
              ...
              <div className="mt-5 flex flex-col gap-3 sm:flex-row">
                <Link to="/report-sample" className="inline-flex items-center justify-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-medium text-slate-700 transition hover:bg-slate-50"><span>View sample report</span><ArrowRight className="h-4 w-4" /></Link>
                {savedReport ? (
                  <button type="button" onClick={() => { setReport(savedReport); setView('report'); }} className="inline-flex items-center justify-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-medium text-slate-700 transition hover:bg-slate-50">
