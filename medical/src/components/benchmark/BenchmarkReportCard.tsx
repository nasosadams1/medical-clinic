  ...
  return (
    <div className="rounded-[1.5rem] border border-border bg-card p-5 shadow-card sm:p-6">
      <div className="flex flex-col gap-4 border-b border-border pb-6 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <div className="type-kicker inline-flex items-center gap-2 rounded-full border border-primary/20 bg-primary/10 px-3 py-1 text-primary">
            {report.isSample ? 'Sample Report' : formatLabel}
          </div>
          <h2 className="type-display-section mt-4 text-foreground">
            {report.overallScore}/100 overall skill score
          </h2>
          <p className="type-body-md mt-2 max-w-2xl text-muted-foreground">{report.summary}</p>
        </div>

        <div className="grid gap-3 sm:min-w-[260px]">
          <div className="rounded-2xl border border-border bg-background/70 px-4 py-3">
            <div className="type-label text-muted-foreground">Correct answers</div>
            <div className="type-title-sm mt-1 text-foreground">
              {report.correctAnswers}/{report.totalQuestions}
            </div>
          </div>
          <div className="rounded-2xl border border-border bg-background/70 px-4 py-3">
            <div className="type-label text-muted-foreground">Role estimate</div>
            <div className="type-title-sm mt-1 text-foreground">{estimation.label}</div>
            <div className="type-body-sm text-muted-foreground">
              {estimation.competencyCoveragePercent}% blueprint coverage
            </div>
          </div>
          <div className="rounded-2xl border border-border bg-background/70 px-4 py-3">
            <div className="type-label text-muted-foreground">Duel readiness</div>
            <div className="type-title-sm mt-1 text-foreground">{report.duelReadiness.label}</div>
            <div className="type-body-sm text-muted-foreground">{report.duelReadiness.confidencePercent}% confidence</div>
          </div>
          <div className="rounded-2xl border border-border bg-background/70 px-4 py-3">
            <div className="type-label text-muted-foreground">Signal confidence</div>
            <div className="type-title-sm mt-1 text-foreground">{report.confidenceBand.label}</div>
            <div className="type-body-sm text-muted-foreground">
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-3">
        ...
        <div className="rounded-2xl border border-border bg-background/70 p-5">
          ...
          <div className="mt-4 rounded-2xl border border-border bg-card px-4 py-4">
            <div className="type-title-sm text-foreground">{estimation.label}</div>
            <p className="type-body-sm mt-2 text-muted-foreground">{estimation.description}</p>
          </div>
        </div>
      </div>
        ...
        <>
          <div className="mt-6 grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
                  ...
                  <div key={dimension.key} className="rounded-2xl border border-border bg-card px-4 py-4">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <div className="type-title-sm text-foreground">{dimension.label}</div>
                        <p className="type-body-sm mt-1 text-muted-foreground">{dimension.description}</p>
                      </div>
                      <div className="type-title-sm text-foreground">{dimension.score}</div>
                    </div>
            ...
            <div className="grid gap-4">
                  ...
                  {report.sectionScores.map((section) => (
                    <div key={section.section} className="rounded-2xl border border-border bg-card px-4 py-4">
                      <div className="flex items-center justify-between gap-3">
                        <div>
                          <div className="type-title-sm text-foreground">{section.label}</div>
                          <div className="type-label mt-1 text-muted-foreground">
                            {section.questionCount} question{section.questionCount === 1 ? '' : 's'}
                          </div>
                        </div>
                        <div className="type-title-sm text-foreground">{section.score}</div>
                      </div>
                      <div className="mt-3 h-2 overflow-hidden rounded-full bg-background">
              ...
              <div className="rounded-[1.5rem] border border-border bg-background/70 p-5">
                <div className="type-label text-muted-foreground">Confidence note</div>
                <div className="mt-4 rounded-2xl border border-border bg-card px-4 py-4">
                  <div className="type-title-sm text-foreground">{report.confidenceBand.label}</div>
                  <p className="type-body-sm mt-2 text-muted-foreground">{report.confidenceBand.description}</p>
                  <div className="type-label mt-3 text-muted-foreground">
              ...
              <div className="rounded-[1.5rem] border border-border bg-background/70 p-5">
                ...
                <div className="mt-4 rounded-2xl border border-border bg-card px-4 py-4">
                  <div className="flex items-center justify-between gap-3">
                    <div className="type-title-sm text-foreground">{report.trustSignal.label}</div>
                    <div className="type-title-sm text-foreground">{report.trustSignal.score}/100</div>
                  </div>
                  <p className="type-body-sm mt-2 text-muted-foreground">{report.trustSignal.description}</p>
                  <div className="mt-3 grid gap-2 sm:grid-cols-2">
                    <div className="rounded-xl border border-border bg-background px-3 py-2">
                      <div className="type-label-tight text-muted-foreground">Blur events</div>
                      <div className="mt-1 text-sm font-semibold text-foreground">{report.telemetrySummary.blurCount}</div>
                    </div>
                    <div className="rounded-xl border border-border bg-background px-3 py-2">
                      <div className="type-label-tight text-muted-foreground">Copy/paste</div>
                  ...
                  {report.trustSignal.suspiciousFlags.length > 0 ? (
                    <div className="mt-3 flex flex-wrap gap-2">
                      {report.trustSignal.suspiciousFlags.map((flag) => (
                        <span
                          key={flag}
                          className="type-label-tight inline-flex rounded-full border border-amber-400/20 bg-amber-500/10 px-2.5 py-1 text-amber-100"
                        >
                          {flag.replace(/_/g, ' ')}
                        </span>
                      ))}
          ...
          <div className="mt-6 grid gap-4 xl:grid-cols-[1.05fr_0.95fr]">
                ...
                {recommendedTracks.length > 0 ? (
                  recommendedTracks.map((track) => (
                    <div key={track!.id} className="rounded-2xl border border-border bg-card px-4 py-4">
                      <div className="type-title-sm text-foreground">{track!.title}</div>
                      <p className="type-body-sm mt-2 text-muted-foreground">{track!.description}</p>
                      <Link
                        to={`/tracks/${track!.id}`}
            ...
            <div className="grid gap-4">
              <div className="rounded-[1.5rem] border border-border bg-background/70 p-5">
                <div className="type-label text-muted-foreground">Suggested lessons</div>
                {suggestedLessons.length > 0 ? (
                  <ul className="mt-4 space-y-3">
                    {suggestedLessons.map((lesson) => (
                      <li key={lesson!.id} className="rounded-2xl border border-border bg-card px-4 py-3">
                        <div className="type-title-sm text-foreground">{lesson!.title}</div>
