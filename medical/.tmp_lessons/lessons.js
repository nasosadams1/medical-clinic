export const calculateXP = (xp, difficulty, lessonIndex, totalLessons, actualTime, // in minutes
baselineTime // in minutes
) => {
...
export const getDifficultyByIndex = (index) => {
    if (index < 17)
        return 'Beginner';
    if (index < 34)
        return 'Intermediate';
...
const isTheoryStep = (step) => step.type === 'theory';
const isQuestionStep = (step) => step.type === 'question';
const compactText = (value) => (value ?? '').replace(/\s+/g, ' ').trim();
const languageFrame = (language) => {
    switch (language) {
        case 'javascript':
...
const buildTheoryStep = (title, lesson, source, reinforcement) => ({
    title,
...
const buildQuestionStep = (lesson, question, index) => ({
    ...question,
...
const rebuildDeploymentLesson = (lesson) => {
...
const rebalanceLessonRewards = (lessons) => {
    const tierProgressByLessonId = new Map();
    ...
    return lessons.map((lesson) => {
        ...
        const normalizedTierProgress = tierProgress.count <= 1 ? 0 : tierProgress.position / (tierProgress.count - 1);
        const rewardBand = LESSON_REWARD_BANDS[lesson.difficulty];
        const targetBaseXp = Math.round(interpolate(rewardBand.baseXp[0], rewardBand.baseXp[1], normalizedTierProgress));
        const targetBaselineTime = roundToHalfMinute(interpolate(rewardBand.baselineTime[0], rewardBand.baselineTime[1], normalizedTierProgress));
        return {
            ...lesson,
            baseXP: targetBaseXp,
...
const enabledPythonLessons = rebalanceLessonRewards(pythonLessons.filter((lesson) => lesson.id !== 'python-advanced-math'));
const deploymentJavascriptLessons = rebalanceLessonRewards(javascriptLessons.map(rebuildDeploymentLesson));
const deploymentCppLessons = rebalanceLessonRewards(cppLessons.map(rebuildDeploymentLesson));
const deploymentJavaLessons = rebalanceLessonRewards(javaLessons.map(rebuildDeploymentLesson));
...
export const formatLessonDisplayName = (lessonId) => {
...
export const getTotalLessonsByLanguage = (language) => {
    return getLessonsByLanguage(language).length;
};
export const getCompletedLessonsByLanguage = (language, completedLessons) => {
