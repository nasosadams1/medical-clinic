...
const PRACTICE_LESSON_OVERRIDES = {
...
const uniqueSnippets = (snippets) => snippets.filter((snippet) => Boolean(snippet && snippet.trim())).filter((snippet, index, list) => list.indexOf(snippet) === index);
const cleanPracticePrompt = (prompt) => prompt.replace(/^Q1\s*\(Coding\)\s*\n?/i, '').trim();
const hasToken = (line, pattern) => pattern.test(line);
const derivePracticeSnippets = (solution) => {
    ...
    const structuralSnippets = lines.flatMap((line) => {
        ...
        const stringLiterals = [...line.matchAll(/"([^"]+)"|'([^']+)'/g)]
            ...
            .map((value) => `"${value}"`);
        if (assignmentMatch)
            snippets.push(`${assignmentMatch[1]} =`);
        if (defMatch)
            snippets.push(`def ${defMatch[1]}`);
        if (classMatch)
            snippets.push(`class ${classMatch[1]}`);
        if (hasToken(line, /\bprint\(/))
            snippets.push('print(');
        if (hasToken(line, /\binput\(/))
            snippets.push('input(');
        if (hasToken(line, /\bint\(/))
            snippets.push('int(');
        if (hasToken(line, /\bfloat\(/))
            snippets.push('float(');
        if (hasToken(line, /\bstr\(/))
            snippets.push('str(');
        if (hasToken(line, /\bbool\(/))
            snippets.push('bool(');
        if (line.includes('if '))
            snippets.push('if ');
        ...
        if (line.includes('except'))
            snippets.push('except');
        if (hasToken(line, /\bwith open\(/))
            snippets.push('with open(');
        if (!hasToken(line, /\bwith open\(/) && hasToken(line, /\bopen\(/))
            snippets.push('open(');
        if (hasToken(line, /\bread\(/))
            snippets.push('read(');
        if (hasToken(line, /\bwrite\(/))
            snippets.push('write(');
...
const TRAP_ERROR_OPTION_BY_LESSON = {
...
const TRAP_EXPLANATION_OVERRIDES = {
...
const applyTrapErrorLabels = (lesson) => {
    ...
    const steps = lesson.content.steps.map((step) => {
        ...
        if (errorIndex < 0 || !replacement)
            return step;
        return {
            ...step,
            options: step.options.map((option) => (option === 'Error' ? replacement : option)),
            explanation: step.correctAnswer === errorIndex
...
const PYTHON_TRANSLATION_FIXES = {
    ...
    'Write a program that prompts the user for two numbers and displays their sum.': 'Write a program that asks the user for two numbers and prints their sum.',
    ...
    'Create class Counter with attribute value = 0 inside constructor and method inc() which increments value by 1. Then create object, call inc() once and display value.': 'Create a Counter class with a value attribute set to 0 in the constructor and an inc() method that increases value by 1. Then create an object, call inc() once, and print value.',
};
const translatePythonLessonText = (value) => {
    if (typeof value === 'string') {
        return ((PYTHON_TRANSLATION_FIXES[pythonLessonTranslations[value] ?? value] ?? pythonLessonTranslations[value] ?? value).replace(/\u00d7/g, '*'));
    }
    if (Array.isArray(value)) {
        return value.map((item) => translatePythonLessonText(item));
    }
    if (value && typeof value === 'object') {
        return Object.fromEntries(Object.entries(value).map(([key, entryValue]) => [key, translatePythonLessonText(entryValue)]));
    }
    return value;
};
const pythonLessonBlueprints = [
    {
...
export const pythonLessons = pythonLessonBlueprints
    .map(makeLesson)
